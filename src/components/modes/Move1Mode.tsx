import { Repeat, Play, Square, AlertTriangle } from "lucide-react";
...
import { startMove1, stop, useSliderStore } from "@/store/sliderStore";
...
export function Move1Mode() {
  ...
  const [velocity, setVelocity] = useState(sliderState.move1Vel);

  const handleStart = async () => {
    const success = await startMove1({ vel: velocity });
    toast({
      title: success ? "Move1 started" : "Move1 failed",
      description: success ? "Periodic movement started" : "Command was rejected by device",
    ...
    <div className="space-y-4">
      ...
      <Card className="bg-card border-border">
        <CardHeader className="pb-3">
          ...
          <p className="text-xs text-muted-foreground">Continuous A-B-A periodic movement.</p>
        </CardHeader>

        <CardContent className="space-y-5">
          <PositionVisualization currentPosition={sliderState.position} pointA={0} pointB={100} isRunning={sliderState.mode === "move1"} />

          {!isCalibrated && (
            <div className="flex items-start gap-2 rounded-md border border-warning/40 bg-warning/10 p-3 text-warning">
              <AlertTriangle className="mt-0.5 h-4 w-4 shrink-0" />
              <p className="text-xs">Calibration required before periodic movement.</p>
            </div>
          )}
